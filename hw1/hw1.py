...
p1_u = [ 0, 4]
p1_v = [-1, 3]
p1_v_plus_u = [p1_u[c]+ p1_v[c] for c in range(len(p1_u))]
p1_v_minus_u =[p1_v[c]- p1_u[c] for c in range(len(p1_u))]

p1_three_v_minus_two_u = [3*p1_v[c]- 2*p1_u[c] for c in range(len(p1_u))]
...
p2_u = [-1,  1, 1]
p2_v = [ 2, -1, 5]
p2_v_plus_u = [p2_v[c] + p2_u[c] for c in range(len(p2_u)) ]
p2_v_minus_u = [p2_v[c]-p2_u[c] for c in range(len(p2_v))]
p2_two_v_minus_u = [2*p2_v[c] - p2_u[c] for c in range(len(p2_v))]
p2_v_plus_two_u = [p2_v[c]+ 2* p2_u[c] for c in range(len(p2_v))]
